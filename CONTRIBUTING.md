...
Thanks for improving Claude Model Optimizer.
...
- `skills/claude-model-optimizer/SKILL.md` contains detailed routing guidance. Put invocation-only mechanics under `skills/claude-model-optimizer/references/`.
...
`tests/sync.sh` fails if the generated file is stale or if the always-on policy becomes too large. The `claude-md` installer also installs the project-local skill so the policy can direct Claude to `/claude-model-optimizer` for detail.
