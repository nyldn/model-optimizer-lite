...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/model-optimizer-lite-test.XXXXXX")"
...
unset MODEL_OPTIMIZER_LITE_MODE
unset MODEL_OPTIMIZER_LITE_TARGET
unset MODEL_OPTIMIZER_LITE_CLAUDE_SKILLS_DIR
...
PIPE_HELP_OUTPUT="$(MODEL_OPTIMIZER_LITE_REPO_URL="invalid://must-not-clone" bash -s -- --help < "$ROOT/install.sh")"
...
test -f "$HOME_DIR/.claude/skills/model-optimizer-lite/SKILL.md"
grep -q "run /model-optimizer-lite" <<< "$USER_INSTALL_OUTPUT"
...
MODEL_OPTIMIZER_LITE_TARGET="$PROJECT_DIR" "$ROOT/install.sh" skill-project
test -f "$PROJECT_DIR/.claude/skills/model-optimizer-lite/SKILL.md"

mkdir -p "$PROJECT_DIR/.claude"
printf '# Project Instructions\n\nKeep this project-specific note.\n\t\nTrailing-whitespace line above must survive.\n' > "$PROJECT_DIR/.claude/CLAUDE.md"

MODEL_OPTIMIZER_LITE_TARGET="$PROJECT_DIR" "$ROOT/install.sh" claude-md
test -f "$PROJECT_DIR/.claude/CLAUDE.md"
test -f "$PROJECT_DIR/.claude/skills/model-optimizer-lite/SKILL.md"
test -f "$PROJECT_DIR/.claude/skills/model-optimizer-lite/references/codex-workflows.md"
...
grep -q "/model-optimizer-lite" "$PROJECT_DIR/.claude/CLAUDE.md"
grep -q "model-optimizer-lite:start" "$PROJECT_DIR/.claude/CLAUDE.md"
...
MODEL_OPTIMIZER_LITE_TARGET="$PROJECT_DIR" "$ROOT/install.sh" claude-md
MODEL_OPTIMIZER_LITE_TARGET="$PROJECT_DIR" "$ROOT/install.sh" claude-md
test "$(grep -c "model-optimizer-lite:start" "$PROJECT_DIR/.claude/CLAUDE.md")" -eq 1
...
printf '\nlocal edit\n' >> "$PROJECT_DIR/.claude/skills/model-optimizer-lite/SKILL.md"
MODEL_OPTIMIZER_LITE_TARGET="$PROJECT_DIR" "$ROOT/install.sh" skill-project
...
grep -q "local edit" "$PROJECT_DIR"/.claude/skill-backups/model-optimizer-lite.backup.*/SKILL.md
! grep -q "local edit" "$PROJECT_DIR/.claude/skills/model-optimizer-lite/SKILL.md"
...
cp -R "$ROOT/skills/model-optimizer-lite/." "$LINK_SKILL_DIR/checkout/"
ln -s ../../checkout "$LINK_SKILL_DIR/.claude/skills/model-optimizer-lite"
MODEL_OPTIMIZER_LITE_TARGET="$LINK_SKILL_DIR" "$ROOT/install.sh" skill-project
test ! -L "$LINK_SKILL_DIR/.claude/skills/model-optimizer-lite"
test -f "$LINK_SKILL_DIR/.claude/skills/model-optimizer-lite/SKILL.md"
...
(
  umask 027
  MODEL_OPTIMIZER_LITE_TARGET="$FRESH_DIR" "$ROOT/install.sh" claude-md
...
printf '# House rules\n\nMarkers look like <!-- model-optimizer-lite:start --> in our docs.\n\n## Secrets\n\nNever commit credentials.\n' > "$PROSE_DIR/.claude/CLAUDE.md"
MODEL_OPTIMIZER_LITE_TARGET="$PROSE_DIR" "$ROOT/install.sh" claude-md
...
if MODEL_OPTIMIZER_LITE_TARGET="$BROKEN_DIR" "$ROOT/install.sh" claude-md >/dev/null 2>&1; then
...
if MODEL_OPTIMIZER_LITE_TARGET="$DIR_DEST" "$ROOT/install.sh" claude-md >/dev/null 2>&1; then
...
MODEL_OPTIMIZER_LITE_TARGET="$LINK_DIR/proj" "$ROOT/install.sh" claude-md
