...
Let Codex use its configured model unless the workflow specifically calls for GPT-5.6 Sol. To pin Sol, add:
...
Use Codex as a second reviewer when another perspective is worth the setup cost. The primary Claude session still inspects every cited finding.
